...
| `POST /batches` | `batchApi.create` | OK | OK | 400/404/409→toast | `homeRoomId` optional (per-batch home room) |
...
| `POST /schedules/generate` | `scheduleApi.generate` (ScheduleGenerator) | OK | OK | 400/404/409/422→toast | 202 + SolveJobResponse; 422 IllegalState when infeasible. Request now carries optional `preAllocations` (`PreAllocationSpec[]`) — bound to the freshly created schedule as teacher-pins before the solver runs |
...
| `POST /pre-allocations` | `preAllocationApi.create` (FE dead — wizard sends `preAllocations` specs inside `POST /schedules/generate` instead) | OK | OK | 400/404/409→toast | — |
| `GET /pre-allocations/{id}` | FE dead (`getById`) | — | OK | — | orphaned client side |
| `GET /pre-allocations/schedule/{scheduleId}` | `preAllocationApi.getBySchedule` (TimetableViewer) | — | OK | — | drives the pre-assigned badge |
| `DELETE /pre-allocations/{id}` | `preAllocationApi.delete` (FE dead) | — | — | 400/404/409→toast | — |
